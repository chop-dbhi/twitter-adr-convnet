...
def crossValidate(parameters, training_tweets_as_words, training_labels, tweet_length, nfolds = _NFOLDS):
    ...
    for idx, b1 in enumerate(bins):
        ...
        _, pred_labels = tweetnet_multilayer_eval.eval(cv_tweets, cv_labels, tweet_length,
                                            vocab, vocab_inv,
...
def train(training_tweets_as_words, training_labels, tweet_length,
          ...
          save_checkpoints = True,
          checkpoints_save_path = CHECKPOINTS_PATH,
          loss_listener = None,
    ...
    dropout_rate_p = parameter_dict.get('DROPOUT_RATE_P', _DROPOUT_RATE_P)
    ...
    with tf.Graph().as_default():
        ...
        if perform_cv_each_batch:
            with tf.variable_scope("cv_loss") as scope:
                # need to put logits in new scope to can be reused
                logits = tweetnet_multilayer.inference(tweets,
                                                NUM_CLASSES,
                ...
                logits_cv = tweetnet_multilayer.inference(tweets_cv,
                                                NUM_CLASSES,
                                                vocab_size, cv_tweet_length, word_embedding_length,
                                                num_maps_per_filter_width,
                                                1.0, L2_CONSTRAINT,
                                                pre_trained_embedding_constant,
                                                filter_widths_by_conv_stack)
        ...
        # calculate loss
        loss = tweetnet_multilayer.loss(logits, labels)
        if perform_cv_each_batch:
            loss_cv = tweetnet_multilayer.loss(logits_cv, labels_cv)
            predictions_op = tweetnet_multilayer.predictions(logits_cv)

        # create train op
        train_op = tweetnet_multilayer.train(loss, global_step, BATCH_SIZE,
...
def load_config(config_file, parameter_mode = SINGLE_PARAMETER_MODE):
    ...
    if parameter_mode==SINGLE_PARAMETER_MODE:
        ...
        if conf.has_section('DebugFilterWidthsByConvStack'):
            opt_dict = confhelp.ConfigSectionMap(conf, 'DebugFilterWidthsByConvStack')
            fwbs = []
            for v in opt_dict.values():
                fwbs.append([int(x) for x in v.split(',')])
            if fwbs:
                params['FILTER_WIDTHS_BY_CONV_STACK'] = fwbs
        return params
    else:
        ...
        if conf.has_option(section, 'NUM_CONV_STACK_PERMUTATIONS'):
            num_stacks = conf.getint(section, 'NUM_CONV_STACK_PERMUTATIONS')
            conv_stacks = []
            for stack_id in range(num_stacks):
                section = 'MultiParameterMode_ConvStack_{0}'.format(stack_id+1)
                if conf.has_section(section):
                    fwbs = []
                    opt_dict = confhelp.ConfigSectionMap(conf, 'MultiParameterMode_ConvStack_{0}'.format(stack_id+1))
                    for v in opt_dict.values():
                        fwbs.append([int(x) for x in v.split(',')])
                    if fwbs:
                        conv_stacks.append(fwbs)
            if conv_stacks:
                parameter_ranges['FILTER_WIDTHS_BY_CONV_STACK']=conv_stacks
...
def main(argv=None):
    ...
    if parameter_mode == SINGLE_PARAMETER_MODE:
        ...
        vocab, vocab_inv = wrangle.build_vocab(training_tweets_as_words)
        tweetnet_multilayer_eval.eval(training_tweets_as_words[0:100], eval_labels, tweet_length,
                                     vocab, vocab_inv,
