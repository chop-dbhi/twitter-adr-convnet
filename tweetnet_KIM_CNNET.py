...
def inference_avg_multi_pretrained(tweets,
                                   num_classes,
                                   tweet_length,
                                   filter_widths, channels_per_filter_width,
                                   dropout_p, l2_lambda,
                                   word_embedding_matrices):
    '''
    builds the tweetnet model - a 1 layer convolutional neural net
    :param tweets: J X K integer valued numpy array where J is number of tweets, K = tweet_length, tweets[j,k] are the indices into the word embedding matrix
    :param tweet_length: length of longest tweets, others are assumed padded to same length
    :param filter_widths: list of integers
    :param channels_per_filter_width: int, number of channels per filter width
    :param dropout_p: dropout regularization probability
    :param word_embedding_matrices: list of word embedding matrices where each
        word_embedding_matrix: V X M real valued numpy array where V is the vocab length and M is the embedding length
        NOTE: the same vocab order should be used across all embedding matrices, ie ebmedding[v] should represent the same word for all embeddings
    ...
    with tf.variable_scope('embedding') as scope:
        embedded_token_channels = []
        for embedding_matrix in word_embedding_matrices:
            embedding_length = embedding_matrix.get_shape()[1].value
            embedded_tokens = tf.expand_dims(tf.nn.embedding_lookup(embedding_matrix, tweets), -1, name=scope.name)
            embedded_token_channels.append((embedded_tokens, embedding_length))

    # define the convolution layer
    conv_layer_output = []
    for filter_width in filter_widths:
        with tf.variable_scope("conv-mp-{0}".format(filter_width)) as scope:
            channel_output = []
            for idx, (et, el) in enumerate(embedded_token_channels):
                with tf.variable_scope("we-channel-{0}".format(idx)) as scope:
                    kernel = _variable_with_weight_decay('weights',
                                                    shape=[filter_width,
                                                            el,
                                                            1,
                                                            channels_per_filter_width],
                                                    stddev=0.1,
                                                    wd = l2_lambda)

                    conv = tf.nn.conv2d(et,
                                    kernel,
                                    strides=[1,1,1,1],
                                    padding='VALID',
                                    name='conv')

                    channel_output.append(conv)


            biases = _variable_on_cpu('biases', [channels_per_filter_width], tf.constant_initializer(0.0))
            conv_sum = channel_output[0]
            for c in channel_output[1:]:
                conv_sum = tf.add(conv_sum, c)
            conv_avg = tf.div(conv_sum, len(embedded_token_channels))
            relu = tf.nn.relu(tf.nn.bias_add(conv_avg, biases), name=scope.name)
            _activation_summary(relu)

            # apply max pooling
            pool = tf.nn.max_pool(relu,
                                  ksize=[1, tweet_length - filter_width + 1, 1, 1],
                                  strides=[1,1,1,1],
                                  padding='VALID',
                                  name='pool')
            #append outputs
    ...
    total_filter_count = channels_per_filter_width * len(filter_widths)
...
def inference(tweets,
              ...
              filter_widths, maps_per_filter_width,
              dropout_p, l2_lambda,
    ...
    :param filter_widths: list of integers
    :param maps_per_filter_width: int, number of channels per filter width
    :param dropout_p: dropout regularization KEEP probability
    ...
    for filter_width in filter_widths:
        with tf.variable_scope("conv-mp-{0}".format(filter_width)) as scope:
            ...
            kernel = _variable_with_weight_decay('weights',
                                                 ...
                                                 stddev=0.1,
                                                 wd = l2_lambda)
            conv = tf.nn.conv2d(embedded_tokens,
                                kernel,
                                strides=[1,1,1,1],
                                padding='VALID',
                                name='conv')
    ...
    # I am concerned there is suboptimal data flow
    #combine outputs from each filter
    total_filter_count = maps_per_filter_width * len(filter_widths)
    conv_layer_output_flat = tf.reshape(tf.concat(3, conv_layer_output), [-1, total_filter_count])
